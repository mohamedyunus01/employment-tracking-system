...
def home3(request):
    ...
    return render(request, 'DigiResume/blog/home3.html', context)

def search(request):
    template='DigiResume/blog/home3.html'
...
class PostListView(ListView):
    model = Post
    template_name = 'DigiResume/blog/home3.html'  # <app>/<model>_<viewtype>.html
...
class UserPostListView(ListView):
    model = Post
    template_name = 'DigiResume/blog/user_posts.html'  # <app>/<model>_<viewtype>.html
    context_object_name = 'posts'
    paginate_by = 2
...
class PostDetailView(DetailView):
    model = Post
    template_name = 'DigiResume/blog/post_detail.html'


class PostCreateView(LoginRequiredMixin, CreateView):
    model = Post
    template_name = 'DigiResume/blog/post_form.html'
...
class PostDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = Post
    success_url = '/'
    template_name = 'DigiResume/blog/post_confirm_delete.html'
...
def about(request):
    return render(request, 'DigiResume/blog/about.html', {'title': 'About'})
    ...
    if request.GET:
        ...
        try:
            ...
            if 'SEV' in id:
                ...
                x=SevaStore.objects.get(seva_code=id).password
            if password==x:
                return redirect(f'/{id}/home3/')
            else:
                flag=False
        except:
            message = 'user does not exist'
    return render(request,'DigiResume/login2.html',{'flag':flag, 'message' : message})
